...
tolerance = 0.259; // PETG shrinking factor
bearing_diameter = 14.6+tolerance;
height = 58;
...
module igus_cut(){
    ...
    difference(){
        union(){
            difference(){
            translate([0,0,4.3])
            cylinder(1,r=(14.80+tolerance)/2+2,$fn=64);

            translate([0,0,4.3+1-0.25])
            cylinder(0.25+1,r1=(14.80+tolerance)/2,r2=(14.80+tolerance)/2+2,$fn=64);

            translate([0,0,4.3+1-1-1])
            cylinder(0.25+1,r1=(14.80+tolerance)/2+2,r2=(14.80+tolerance)/2,$fn=64);

            }
            difference(){
                translate([0,0,4.3+16.4])
                cylinder(1,r=(14.80+tolerance)/2+2,$fn=64);
                
                translate([0,0,4.3+1-0.25+16.4])
                cylinder(1.25,r1=(14.80+tolerance)/2,r2=(14.80+tolerance)/2+2,$fn=64);
                
                translate([0,0,4.3+1-2+16.4])
                cylinder(1.25,r1=(14.80+tolerance)/2+2,r2=(14.80+tolerance)/2,$fn=64);
            }
            //translate([0,0,4.3+16.4])
            //cylinder(1,r=14.80/2+2,$fn=64);
        }
        translate([0,0,1])
        cylinder(24,r=(14.80+tolerance-0.1)/2,$fn=64);
    }
...
module vertical_bearing_holes_org(){
...
module vertical_bearing_holes(){
    ...
    difference(){
        vertical_bearing_holes_org();

        translate([0,0,-1+0.2])
        igus_cut();

        translate([0,0,height-24-1-0.2])
        igus_cut();
    }
...
translate([0,5,12])
rotate([90,0,0])
import("RJ4JP_01_08_2.stl");
*/

/*
translate([0,15,0])
cylinder(height,r=5,$fn=64);
*/
